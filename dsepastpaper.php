...
<?php 
	...
	$db = new Database();
		...
		$query = "SELECT * FROM questions WHERE paper = 1 AND id IN (SELECT question_id FROM relation WHERE topic_id = $id)";
		$paper1 = $db->select($query);
		
		$query = "SELECT * FROM questions WHERE paper = 2 AND id IN (SELECT question_id FROM relation WHERE topic_id = $id)";
...
<div class="panel panel-default">
 ...
 <table class="table">
    ...
    <?php while($row = $paper1->fetch_assoc()) :?>
          <tr>
            ...
            <td><a href="http://hk.yahoo.com">Q<?php echo $row['number']?></a></td>
 ...
 <?php else:?>
  ...
  There are no related questions yet
...
<div class="panel panel-default">
  ...
  <li class="list-group-item"><a href="dsepastpaper.php?id=<?php echo $row['id']?>"><?php echo $row['name']?></a></li>
